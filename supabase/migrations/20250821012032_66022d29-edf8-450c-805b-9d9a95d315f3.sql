-- Создание таблиц для полноценного бэкенда

-- Типы ролей пользователей
CREATE TYPE public.user_role AS ENUM ('admin', 'freelancer', 'outsourcer', 'founder', 'investor');
...
('super_admin_credentials', '{"login": "admin", "password_hash": "$2b$10$8K8KqKq5K5K5K5K5K5K5KOuGGGGGGGGGGGGGGGGGGGGGGGGGGG", "email": "admin@investex.ru"}', 'Credentials for super admin access'),
...
((SELECT id FROM public.skill_categories WHERE name = 'Разработка'), 'React', 'Frontend разработка на React'),
((SELECT id FROM public.skill_categories WHERE name = 'Разработка'), 'Node.js', 'Backend разработка на Node.js'),
((SELECT id FROM public.skill_categories WHERE name = 'Разработка'), 'Python', 'Разработка на Python'),
((SELECT id FROM public.skill_categories WHERE name = 'Дизайн'), 'UI/UX Design', 'Дизайн пользовательских интерфейсов'),
((SELECT id FROM public.skill_categories WHERE name = 'Маркетинг'), 'SMM', 'Продвижение в социальных сетях'),
((SELECT id FROM public.skill_categories WHERE name = 'Бизнес'), 'Business Analysis', 'Бизнес-анализ'),
((SELECT id FROM public.skill_categories WHERE name = 'Финансы'), 'Financial Modeling', 'Финансовое моделирование');