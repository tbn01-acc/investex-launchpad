...
-- Update profiles table to use new role system (keep old role for now)
...
-- Now we can drop the old role column and rename new one
ALTER TABLE public.profiles DROP COLUMN role;
ALTER TABLE public.profiles RENAME COLUMN new_role TO role;

-- Recreate the policy with correct reference
CREATE POLICY "Superadmins can manage admin accounts" 
ON public.admin_accounts 
FOR ALL
USING (EXISTS ( SELECT 1
   FROM admin_accounts
  WHERE ((admin_accounts.id = auth.uid()) AND (admin_accounts.role = 'superadmin'::user_role))));
